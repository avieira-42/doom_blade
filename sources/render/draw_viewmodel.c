...
/*   Updated: 2026/04/07 20:42:32 by adeimlin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	stt_fov_draw(t_mat32 frame, t_player *player, t_quad p_icon)
{
	...
	line_dst.x.i = p_icon.center.x.i + (int32_t)(player->cam.dir.x.f * 8.0f);
	line_dst.y.i = p_icon.center.y.i + (int32_t)(player->cam.dir.y.f * 8.0f);
	draw_line(frame, p_icon.center, line_dst, RADAR_FOV_COLOR);
}
...
void	cub_draw_viewmodel(t_mat32 frame, t_player *player, t_game *game, long dt)
{
	...
	x_tmp = ((r_width - player->viewmodel[0]->texture.width) / 2) + 5;	// TODO: defines
	y_tmp = r_height - player->viewmodel[0]->texture.height;
	cub_draw_sheet(frame, player->viewmodel[0], x_tmp, y_tmp);
	if (player->viewmodel[1] != NULL)
	{
		cub_draw_sheet(frame, player->viewmodel[1], 0, 195);
