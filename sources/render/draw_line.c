...
void	draw_line(t_img *img, t_vec3 p0, t_vec3 p1)
{
	...
	const t_vec3	delta = {.x.f = (double)(p1.x.i - p0.x.i) / (len + 1),
		.y.f = (double)(p1.y.i - p0.y.i)
		/ (len + 1), .z.u = (255u << 16) / (len + 1)};
	t_vec3			vec;
	uint32_t		i;
	...
	while (i <= len)
	{
		if (vec.x.f >= 0.0f && vec.y.f >= 0.0f
			&& vec.x.f < SCREEN_WIDTH && vec.y.f < SCREEN_HEIGHT)
			stt_putrgb(img, vec.x.f, vec.y.f,
				stt_clerp(p0.z.u, p1.z.u, vec.z.u >> 16));
		vec.x.f += delta.x.f;
		vec.y.f += delta.y.f;
