...
static
int	stt_load_texture(t_xvar *mlx, t_str path, size_t count, t_mat32 *dst)
{
	...
	return (cub_read_xpm(mlx, dst, path.ptr, NULL));
}
...
t_sheet cub_readsheet(t_game *game, const char *base_path, size_t count, long frame_time)
{
	...
	if (stt_load_texture(game->mlx, path, 0, &tex) < 0)
		return ((t_sheet){0});
	...
	while (i < count)
	{
		if (stt_load_texture(game->mlx, path, i, &tex) < 0)
			return (free(tex.ptr), (t_sheet){0});
		tex.ptr += sheet.frame_size;
		i++;
	}
	return (sheet);
